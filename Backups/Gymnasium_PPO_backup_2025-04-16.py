...
class EyeSimEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.int8) # turn left or right
    ...
    def step(self, action):
        ...
        eyesim_set_robot_speed(int(action[0])) # action[0] is angular motor speeds.
    ...
    def _calculate_reward(self, init_position, after_position):
        if abs(after_position) < abs(init_position):
            reward = 1.0
        elif abs(after_position) > abs(init_position):
            reward = -1.0
        else:
            reward = 0.0
        return reward
...
def eyesim_set_robot_speed(direction):
    ...
    VWSetSpeed(0,speed*direction*2) # 10 or -10 angular velocity
    OSWait(40) # 5 ms
    VWSetSpeed(0,0) # stop from moving

def eyesim_get_observation():
    img = CAMGet() # get image from camera
    processed_img = np.transpose(image_processing(img), (2,0,1)) # process image
...
def eyesim_reset():
    ...
    if angle < 0:
        angle = 360 + angle
    angle_variation = randint(-50,50) # random angle variation
...
def train():
    ...
    model.learn(total_timesteps=10000, progress_bar = True, tb_log_name="PPO")
    model.save(f"{models_dir}/PPO-Eyesim")
...
def load():
    ...
    model_path = f"{models_dir}/PPO-Eyesim.zip"
