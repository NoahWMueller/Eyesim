...
def go_to(dir):
    ...
    turn = x = y = phi = dist = AngSpeed=0
    #int turn, x,y,phi, dist, L,F,R
    AngSpeed = 0
    ...
    VWSetPosition(0,0,0)
    while True: # adjust angular speed
        L = PSDGet(1)
        F = PSDGet(2)
        R = PSDGet(3)
        LCDSetPrintf(15,0, "L%4d, F%4d, R%4d.", L, F, R)
        if L<WALLD: AngSpeed -= 1
        if R<WALLD: AngSpeed += 1
        LCDSetPrintf(17,0, "AngSpeed %4d.", AngSpeed)

        VWSetSpeed(SPEED, AngSpeed)    # go one step 
        #VWGetPosition(&x,&y,&phi)
        VWGetPosition()
        dist = int(sqrt(x*x+y*y))
        print("x%d, y%d, phi%d dist%4d.\n" % (x, y, phi, dist))
        LCDSetPrintf(14,0, "X%4d, Y%4d, Phi%4d, D%4d.",x,y,phi,dist)
        if dist>=DIST-5: break # 5mm margin #while (dist<DIST-5) 
