...
# Modified and improved lib.py by Noah Mueller (23356892)
# Use this file on the physical Eyebot Robot not for simulator
...
if sys.platform == 'linux2' or sys.platform == 'linux': # Linux
...
lib.RADIOReceive.argtypes = [ctypes.POINTER(ctypes.c_int), ctypes.c_char_p, ctypes.c_int]
...
def RADIOSend(idnum, mes):
	return lib.RADIOSend(ctypes.c_int(idnum), bytes(mes.encode('ascii')))
def RADIOReceive():
	IDNUM = ctypes.c_int()
	BUF = (ctypes.c_char*1024)()  # max message size 1024
	lib.RADIOReceive(IDNUM, BUF, ctypes.c_int(1024))
	return [IDNUM.value , BUF.value.decode('ascii')]
def RADIOCheck():
	return lib.RADIOCheck()
def RADIOStatus():
	NUM    = ctypes.c_int()
	IDLIST = (ctypes.c_int*256)()
	NUM = lib.RADIOStatus(IDLIST)
	return [NUM,IDLIST]
def RADIORelease():
